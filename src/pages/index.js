    ...
    <div className={styles.normasl}>
      <div className={styles.pc}>
        <div className={styles.headerText}>
          <img className={styles.headerTextImg} src='http://pxczv9bs6.bkt.clouddn.com/4b2bde58-8bab-4b46-bb9e-12e658445ca9'/>
          <div style={{marginTop:'15rem'}}><img style={{width:'202px',height:'76px'}} src='http://pxczv9bs6.bkt.clouddn.com/4c57d5b6-5a2e-4b85-afc9-11c0575709bb'/></div>
          ...
          <div className={styles.swiper}>
            <div className="swiper-container container2">
              <div className="swiper-wrapper">
                {articles.map(article=><div  key={article.id}  className="swiper-slide"> <img className={styles.swiperImg} onClick={()=>router.push(`/pc/info/${article.id}`)}  src={article.image}/></div>)}
              </div>
        ...
        <div>
          ...
          <div> <img style={{paddingTop:'5rem'}} src='http://pxczv9bs6.bkt.clouddn.com/c1dbbf88-9dd1-4bed-a117-875ab1cf5056'/></div>
          <img style={{marginTop:'10rem'}} src='http://pxczv9bs6.bkt.clouddn.com/40564633-3368-4334-823e-2a3dc9c56721'/>
        </div>
      ...
      <div className={styles.phone}>
        <div>
          <div className={styles.headerText}>
            <img className={styles.headerTextImg} src='http://pxczv9bs6.bkt.clouddn.com/4b2bde58-8bab-4b46-bb9e-12e658445ca9'/>
            <div style={{marginTop:'2rem'}}><img style={{width:'7rem',height:'3rem'}} src='http://pxczv9bs6.bkt.clouddn.com/4c57d5b6-5a2e-4b85-afc9-11c0575709bb'/></div>
              {/*{classifys.map(classify=><div className={styles.privilegeList}>*/}
              {/*  <img src={classify.image}/>*/}
              {/*  <div>{classify.name}</div>*/}
              {/*</div> )}*/}
              <div className={styles.swiper}>
                <div className="swiper-container container3">
                  <div className="swiper-wrapper">
                    {classifys.map(classify=><div onClick={()=>{router.push('/pc/privilege')}} key={classify.id} className={styles.privilegeList}>
            ...
            <img style={{width:'20rem',marginTop:'5rem'}} src='http://pxczv9bs6.bkt.clouddn.com/7c587ca9-160a-4c7e-af86-68b8d549f98e'/>
            <div>
              <div className={styles.headerHav}>
                {classifys.map(classify=> <div onClick={()=>setPage({...page,...{classifyId:classify.id}})} className={styles.button+' '+(page.classifyId===classify.id?styles.buttonActive:'')}>{classify.name}</div>)}
              </div>
          ...
          <div className={styles.centerDiv}>
            {articles.map((article,index)=>index<4&&<div key={article.id} className={styles.phoneImagesContent}>
              <img onClick={()=>router.push(`/pc/info/${article.id}`)} style={{width:'12rem',height:'14rem'}} src={article.image}/>
              <div style={{textAlign:'left',color:'#fff',padding:'1rem'}}>{article.title&&(article.title.length>5?article.title.substring(0,5):'')}</div>
            </div>)}
            <div className={styles.startShare} onClick={()=>router.push('/pc/push')}>开始分享</div>
          </div>
          <div>
          ...
          <img style={{marginTop:'10rem',width:'100%'}} src='http://pxczv9bs6.bkt.clouddn.com/40564633-3368-4334-823e-2a3dc9c56721'/>
          <div className={styles.footer}>
